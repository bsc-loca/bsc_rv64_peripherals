...
module plic_interface #(
    ...
    reg_bus_if.in external_bus_io  // the bus
);
  ...
  funct_t funct;  // the address mapping will primarily check what
  ...
  logic [7:0] ena_bundles_d[num_gateway_bundles][NUM_TARGETS][DATA_WIDTH/8];
  logic [7:0] ena_bundles_q[num_gateway_bundles][NUM_TARGETS][DATA_WIDTH/8];
  ...
  assign pending_array_tmp[0] = 1'b0;
  for (genvar i = 1; i < NUM_GATEWAYS + 1; ++i) begin
    assign pending_array_tmp[i] = pending_array_i[i-1];
  end
  ...
  for (genvar bundle = 0; bundle < num_gateway_bundles; bundle++) begin
    for (genvar ip_bit = 0; ip_bit < DATA_WIDTH; ip_bit++) begin
      if ((bundle * DATA_WIDTH + ip_bit) < NUM_GATEWAYS + 1) begin
        assign irq_pending_bundle[bundle][ip_bit] = pending_array_tmp[bundle*DATA_WIDTH+ip_bit];
      end else begin
  ...
  for (genvar i = 1; i < NUM_GATEWAYS + 1; ++i) begin
    for (genvar target = 0; target < NUM_TARGETS; ++target) begin
      assign irq_enables_o[i-1][target] = irq_enables_tmp[i][target];
    end
  end
      ...
      for (genvar byte_in_word = 0; byte_in_word < DATA_WIDTH / 8; byte_in_word++) begin
        for (genvar enable_bit = 0; enable_bit < 8; enable_bit++) begin
          if ((bundle * DATA_WIDTH + byte_in_word * 8 + enable_bit) < NUM_GATEWAYS + 1) begin
            assign irq_enables_tmp[bundle * DATA_WIDTH + enable_bit + byte_in_word * 8][target] = ena_bundles_q[bundle][target][byte_in_word][enable_bit];
          end
        end
      end
    end
  ...
  always_comb begin : proc_address_map
    ...
    if (word_offset == '0) begin
      ...
      if (page_address[13:0] == 0) begin  // we access the gateway priority bits
        ...
        if (page_word_offset <= NUM_GATEWAYS && page_word_offset > 0)
        begin //the gateway in question exists
      ...
      IEB: begin
        ...
        if (read_active != 0) begin
          for (integer byte_in_word = 0; byte_in_word < DATA_WIDTH / 8; byte_in_word++) begin
            external_bus_io.rdata[8*(byte_in_word)+:8] = ena_bundles_q[page_offset[6:$clog2(bpw)]]
                [({page_address[8:0], page_offset[11:7]}-64)][byte_in_word];
          end
          external_bus_io.ready = 1;
          // write case
        end else if (write_active != 0) begin
          for (integer byte_in_word = 0; byte_in_word < DATA_WIDTH / 8; byte_in_word++) begin
            if (write_active[byte_in_word]) begin
              ena_bundles_d[page_offset[6:$clog2(bpw)]][({page_address[8:0], page_offset[11:7]}-64)]
                  [byte_in_word] = external_bus_io.wdata[8*(byte_in_word)+:8];
            end
          end
          external_bus_io.ready = 1;
        end
